...
syn keyword  goStatement fallthrough range
syn keyword  goStatement return

syn keyword     goPreProc        import as
syn keyword     goPreCondit      package
...
syn keyword     goTypeDef           var map type
...
syn keyword     goBuildinPkg     main fmt os
" Builtin functions
...
syn match       goDottedName     "[a-zA-Z_][a-zA-Z0-9_]*\(\.[a-zA-Z_][a-zA-Z0-9_]*\)*" display contained
syn match       goDot            "\." display containedin=goDottedName:
syn match       goSemicon         "\;"

syn match       goPointer         "\*[a-zA-Z_][a-zA-Z0-9_]*"
syn match       goConstant        "[A-Z][0-9]"	
...
hi def link goDot           SpecialChar
hi def link goSemicon       SpecialChar
hi def link goPreProc       PreProc
hi def link goPreCondit     PreCondit
hi def link goBuildinPkg    Identifier
hi def link goTypeDef       TypeDef
hi def link goCompare       Compare
"hi def link goCompare       Operator
hi def link goPointer       Pointer
